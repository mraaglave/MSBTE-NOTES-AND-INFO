---
description: Automatically inject the YouTube Subscribe Promo Box into every new blog post
---

# Rule: Always Add YouTube Subscribe Promo

Whenever creating a NEW blog post for the MSBTE Notes & Info website (inside the `/Blog/` directory), you MUST automatically include the "Subscribe to Our YouTube Channel" promo box. 

Do not ask the user for permission. Just insert it.

## Injection Location
The promo box should be placed immediately after the `TL;DR` block, or generally at the top of the article content just inside the `<div class="prose prose-blue max-w-none">` container.

## Promo Box HTML
You must use this exact HTML snippet:
