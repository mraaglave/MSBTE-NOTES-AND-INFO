...
fs.readdir(blogDir, (err, files) => {
    ...
    files.forEach(file => {
        if (path.extname(file) === '.html') {
            ...
            if (content.includes('Subscribe to Our YouTube Channel!')) {
                return;
            }

            const target = '<div class="prose prose-blue max-w-none">';
            if (content.includes(target)) {
                content = content.replace(target, target + promoBoxHTML);
                fs.writeFileSync(filePath, content, 'utf8');
                count++;
                console.log(`Updated ${file}`);
            } else {
                const target2 = '<div class="prose max-w-none">';
                if (content.includes(target2)) {
                    content = content.replace(target2, target2 + promoBoxHTML);
                    fs.writeFileSync(filePath, content, 'utf8');
                    count++;
                    console.log(`Updated ${file}`);
                }
